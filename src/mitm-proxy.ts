...
export function attachMitmProxy(
  ...
  server.on('connect', (req: http.IncomingMessage, clientSocket: net.Socket, head: Buffer) => {
    ...
    if (!match) {
      if (config.proxy.discovery) {
        // Discovery MITM: intercept TLS, inspect headers, forward as-is
        handleDiscoveryMitm(hostname, port, clientSocket, head, certManager);
      } else {
        // Plain TCP passthrough (no MITM, no inspection)
